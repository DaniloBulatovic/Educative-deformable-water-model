...
Shader::SetUniform2f(const std::string& uniform, const glm::vec2& v) const {
    glUniform2f(glGetUniformLocation(mId, uniform.c_str()), v.x, v.y);
}
...
void
Shader::SetUniform4f(const std::string& uniform, const glm::vec4& v) const {
    glUniform4f(glGetUniformLocation(mId, uniform.c_str()), v.x, v.y, v.z, v.w);
}

void
Shader::SetUniform1fv(const std::string& uniform, const std::vector<float>& v) const {
    glUniform1fv(glGetUniformLocation(mId, uniform.c_str()), v.size(), &v[0]);
}

void
Shader::SetUniform3fv(const std::string& uniform, const std::vector<glm::vec3>& v) const {
    glUniform3fv(glGetUniformLocation(mId, uniform.c_str()), v.size(), &v[0][0]);
}
